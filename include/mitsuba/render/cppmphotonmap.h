 ...
 * \ingroup librender
 */
class MTS_EXPORT_RENDER CPPMPhotonMap : public SerializableObject {
public:
     ...
     */
    CPPMPhotonMap(size_t photonCount = 0);

    /**
     * \brief Unserialize a photon map from a binary data stream
     */
    CPPMPhotonMap(Stream *stream, InstanceManager *manager);
    ...
    /// Execute query, see \ref PointKDTree for details
    template <typename Functor> size_t executeQuery(const Point &p,
                Float searchRadius, Functor &functor) const {
        return m_kdtree.executeQuery(p, searchRadius, functor);
    }
    //! @}
    // =============================================================
